...
fn _list_dir(
    path: PathBuf,
) -> Result<Vec<FileStructure>,io::Error> {
    let mut vec = Vec::new();
    ...
    for path_result in fs::read_dir(&path)? {
        ...
        let relative_path: String = full_path.strip_prefix(path.clone()).ok().unwrap().to_str().unwrap().into();
        let context_val: String = if full_path.is_dir() { "folder".into() } else { "file".into() };
